...
public abstract class CallHelper {
	...
	public static <T> TaskResult<T> perform(AtteqServiceLayer asl, Task task, Class<T> c) throws ServiceCallException {
		TaskResultFactory<T> f = new TaskResultFactory<T>();
		return asl.perform(task, new JsonResultTransformer<T, TaskResult<T>>(f), TypeFactory.defaultInstance().constructType(c));
	}

	public static <T> TaskResult<T> performWithErrorDecorator(AtteqServiceLayer asl, Task task, JavaType t) throws ServiceCallException {
		return asl.perform(task, new JsonResultWithErrorTransformer<T>(), t);
	}

	public static <T> TaskResult<T> performWithErrorDecorator(AtteqServiceLayer asl, Task task, Class<T> c) throws ServiceCallException {
		return asl.perform(task, new JsonResultWithErrorTransformer<T>(), TypeFactory.defaultInstance().constructType(c));
	}

	public static <T> GenericResult<T> perform(AtteqServiceLayer asl, Performer performer, JavaType t) throws ServiceCallException {
	...
	public static <T> GenericResult<T> perform(AtteqServiceLayer asl, Performer performer, Class<T> c) throws ServiceCallException {
		GenericResultFactory<T> f = new GenericResultFactory<T>();
		return asl.perform(performer, new JsonResultTransformer<T, GenericResult<T>>(f), TypeFactory.defaultInstance().constructType(c));
	}
