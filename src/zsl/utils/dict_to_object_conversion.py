    ...
    for field_name, field_value in viewitems(dict_data):
        if isinstance(field_value, (type(None), str, int, float, bool)):
            if field_name in hints[DATE_DATA] and field_value is not None:
                d = datetime.strptime(field_value, hints[DATE_DATA][field_name]).date()
                setattr(target, field_name, format_date_portable(d))
            elif field_name in hints[DATETIME_DATA] and field_value is not None:
                d = datetime.strptime(field_value, hints[DATETIME_DATA][field_name])
            ...
            related_cls = hints[RELATED_FIELDS][field_name][RELATED_FIELDS_CLASS]
            related_hints = hints[RELATED_FIELDS][field_name].get(RELATED_FIELDS_HINTS)
