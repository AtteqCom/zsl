...
ZslTestConfiguration = namedtuple('ZslTestConfiguration', ['app_name', 'version', 'container', 'profile'])
    ...
    def setUpClass(cls):
        ...
        if cls.ZSL_TEST_CONFIGURATION is None:
            raise InvalidConfigurationException(
                "Please give a test container specification via 'container' class variable.")

        config: ZslTestConfiguration = cls.ZSL_TEST_CONFIGURATION
        if config.profile:
            set_profile(config.profile)
        app = Zsl(config.app_name + "-test", version=config.version, modules=config.container.modules())
        logging.getLogger(config.app_name).debug("ZSL test app created {0}.".format(app))
